...
class Branch(Base):
    ...
    # 브로제이 얼굴 등록 강제 - True면 화순점 같은 broj 지점에서 회원가입 시 face 필수.
    # 흐름: S3 PUT → PATCH /jcustomer/profile-image → POST /jmember/faceid/add.
    # 실패 시 HiFIS 가입 차단 (지금은 broj cleanup API 미발견 → 브로제이 잔존 가능).
    broj_face_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default="false", default=False,
    )
